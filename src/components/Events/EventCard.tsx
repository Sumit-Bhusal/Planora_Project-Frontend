...
import { Calendar, MapPin, Users, DollarSign, Tag, Edit, BarChart3, Share2, MoreVertical } from 'lucide-react';
...
const EventCard: React.FC<EventCardProps> = ({ 
  ...
  const getAvailabilityColor = () => {
    const availability = ((event.capacity - event.registeredCount) / event.capacity) * 100;
  ...
  return (
    <>
      <Card className={`overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-300 ${
        variant === 'organizer' ? 'border-primary-200 dark:border-primary-700' : ''
      }`} hover>
          ...
          <div className="space-y-2 mb-4">
            ...
            {variant === 'organizer' && (
              <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
                ...
                <span>Revenue: NPR {(event.registeredCount * event.price).toLocaleString()}</span>
              </div>
            )}
          </div>
          ...
          {event.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {event.tags.slice(0, 3).map((tag, index) => (
                <div key={index} className="flex items-center bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-1 rounded-full text-xs border border-gray-200 dark:border-gray-600">
